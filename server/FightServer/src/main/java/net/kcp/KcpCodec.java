...
public class KcpCodec {
    private static final Logger LOGGER = Logger.getLogger(KcpCodec.class);

    public static List<MessageHandler> decoder(ByteBuf buf, int[] decryptKey) {
        try {
            List<MessageHandler> msgs = new ArrayList<>();
            ...
            while (buf.readableBytes() >= 6) {
                ...
                int bodyLength = length - (KcpConstant.PACKET_TAG_LENGTH + KcpConstant.PACKET_LENGTH
                        + KcpConstant.PACKET_CHECK_SUM + KcpConstant.PACKET_COMMAND_ID);
                ...
                if (buf.isReadable()) {
                    ...
                    buf.readBytes(body);
                    messageHandler = MessageHandlerFactory.getInstance().fetchHandler(messageId,
                ...
                if (messageHandler == null) {
                    LOGGER.error("init MessageHandler failed. id:" + messageId);
                } else {
                    msgs.add(messageHandler);
                }
    ...
    public static ByteBuf encoder(SMessage msg, int[] encryptKeys) {
        ...
        int contentLength = KcpConstant.PACKET_TAG_LENGTH + KcpConstant.PACKET_LENGTH
                + KcpConstant.PACKET_CHECK_SUM + KcpConstant.PACKET_COMMAND_ID
                + KcpConstant.PACKET_STATUS_CODE + dataLength;
