...
public class KcpThread extends Thread {
    private static final Logger LOGGER = Logger.getLogger(KcpThread.class);
        ...
        while (this.running) {
            try {
                ...
                while (!this.inputs.isEmpty()) {
                    ...
                    if (content == null || content.readableBytes() < 24) {
                        return;
                    }
                    // conv应该在客户端第一次建立时获取
                    int conv = content.readInt();
                    content.resetReaderIndex();

                    KcpOnUdp ku = this.kcps.get(dp.sender());
                    if (ku == null) {
                        ku = new KcpOnUdp(this.out, dp.sender(), local, this.listerner);// 初始化
                        ku.setSessionValue(KcpConstant.DECRYPTION_KEYS,
                                EncryptionAndDecryptionUtil.getDefaultCustomDecryptionKeys());
                        ku.setSessionValue(KcpConstant.ENCRYPTION_KEYS,
