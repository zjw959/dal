/**
 * KCP - A Better ARQ Protocol Implementation skywind3000 (at) gmail.com, 2010-2011 Features: +
 * Average RTT reduce 30% - 40% vs traditional ARQ like tcp. + Maximum RTT reduce three times vs
 * tcp. + Lightweight, distributed as a single source file.
 */
package net.kcp;
...
public class Kcp {
    ...
    public int send(ByteBuf buffer) {
        ...
        if (this.stream && !this.snd_queue.isEmpty()) {
            ...
            if (seg.data != null && seg.data.readableBytes() < mss) {
                ...
                int extend =
                        (buffer.readableBytes() < capacity) ? buffer.readableBytes() : capacity;
   ...
   public int input(ByteBuf data) {
    ...
    /**
     * update state (call it repeatedly, every 10ms-100ms), or you can ask ikcp_check when to call
     * it again (without ikcp_input/_send calling).
     *
