...
public class KcpOnUdp {
    ...
    private final Map<Object, Object> sessionValue;
     ...
     * @param listerner 监听
     */
    public KcpOnUdp(Output out, InetSocketAddress remote, InetSocketAddress local,
            KcpListerner listerner) {
        this.listerner = listerner;
        kcp = new Kcp(out, remote);
        received = new LinkedBlockingQueue<>();
        sendList = new LinkedBlockingQueue<>();
        this.sessionValue = new HashMap<>();
        this.remote = remote;
        this.local = local;
    }
    ...
    void update() {
        ...
        while (!this.received.isEmpty()) {
            ...
            if (errcode != 0) {
                this.closed = true;
                this.release();
                this.listerner.handleException(
                        new IllegalStateException("input error : " + errcode), this);
        ...
        while (!this.sendList.isEmpty()) {
            ...
            if (errcode != 0) {
                this.closed = true;
                this.release();
                this.listerner.handleException(new IllegalStateException("send error : " + errcode),
                        this);
                this.listerner.handleClose(this);
                return;
            }
        ...
        // check timeout
        if (this.timeout > 0 && lastTime > 0
                && System.currentTimeMillis() - lastTime > this.timeout) {
            this.closed = true;
            this.release();
            this.listerner.handleClose(this);
        }
     ...
     * @return
     */
    public Map<Object, Object> getSessionValueMap() {
        return sessionValue;
    }
     ...
     * @return
     */
    public Object getSessionValue(Object k) {
        return this.sessionValue.get(k);
    }
     ...
     * @return
     */
    public Object setSessionValue(Object k, Object v) {
        return this.sessionValue.put(k, v);
    }
    ...
    public boolean containsSessionKey(Object k) {
        return this.sessionValue.containsKey(k);
    }
    ...
    public boolean containsSessionValue(Object v) {
        return this.sessionValue.containsValue(v);
    }
