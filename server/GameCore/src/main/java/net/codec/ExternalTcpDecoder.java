...
public class ExternalTcpDecoder extends ByteToMessageDecoder {
	private static final Logger LOGGER = Logger
			.getLogger(ExternalTcpDecoder.class);
	...
	private int[] getDecryptionKeys(ChannelHandlerContext ctx) {
		...
		// 取得解密密钥
		Attribute<int[]> decKeys = channel
				.attr(IChannelConstants.DECRYPTION_KEYS);
		int[] dKeys = null;
		if ((dKeys = decKeys.get()) == null) {
			// 获取默认解密密钥
			Attribute<int[]> decryptionKeysAttribute = channel
					.attr(IChannelConstants.DECRYPTION_KEYS_ATTRIBUTE_KEY);
			int[] decryptionKeys = null;
			if ((decryptionKeys = decryptionKeysAttribute.get()) == null) {
				// 设置默认解密密钥
				decryptionKeys = EncryptionAndDecryptionUtil
						.getDefaultCustomDecryptionKeys();
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in,
		...
		try {
			...
			System.arraycopy(decryptKey, 0, copyDecryptKeys, 0,
					copyDecryptKeys.length);
			// 包头解密操作
			validateData = EncryptionAndDecryptionUtil.decryptCustom(
					validateData, copyDecryptKeys);
			// 包头BUF
			...
			int fixLength = length
					- ProtocolConstants.FLAG_AND_LENGTH_DATA_SIZE;
			...
			EncryptionAndDecryptionUtil.decryptCustom(in.nioBuffer(), 0,
					length, decryptKey);
			...
			int checksumContentLength = length
					- ProtocolConstants.CHECKSUM_SKIP_SIZE_FOR_CONTENT;
			bodyLength = length - ProtocolConstants.DEFAULT_HEADER_SIZE;
			short calculateCheckSum = CheckSumUtil.calculate(in.nioBuffer(), 0,
...
//			if (in.isReadable()) {
				...
				in.readBytes(body);
				messageHandler = MessageHandlerFactory.getInstance()
						.fetchHandler(msgId, System.currentTimeMillis(), body);
//			}
			if (messageHandler == null) {
				LOGGER.error("init MessageHandler failed. id:" + msgId
						+ " , ctx:" + ctx);
			} else {
				out.add(messageHandler);
			}
            num.getAndIncrement();
		} catch (Exception ex) {
			LOGGER.error(ex, ex);
