...
public class CacheHandler {
    ...
    public CacheHandler(ICacheManager cacheManager, AbstractScriptParser scriptParser, AutoLoadConfig config,
            ICloner cloner, DBLoadHanler dbLoadHanler) {
        this.processing = new ConcurrentHashMap<CacheKeyTO, ProcessingTO>(config.getProcessingMapSize());
        this.cacheManager = cacheManager;
        this.config = config;
        this.cloner = cloner;
        this.autoLoadHandler = new AutoLoadHandler(this, config, dbLoadHanler);
        ...
        if (scriptParser.isCacheable(cache, pjp.getTarget(), arguments, result)) {
            ...
            try {
                ...
                if (null != autoLoadTO) {
                    ...
                    .setExpireAt(cacheWrapper.getExpireAt())        
    ...
    public Object proceed(CacheAopProxyChain pjp, Cache cache) throws Throwable {
        ...
        try {
            cacheWrapper = this.get(cacheKey, method, pjp.getTarget(), arguments);// 从缓存中获取数据
        } catch (Exception ex) {
            log.error(ex.getMessage(), ex);
        }
        if (log.isTraceEnabled()) {
        ...
        if (null != cacheWrapper && !cacheWrapper.hasExpired()) {
            ...
            try {
                ...
                if (null != autoLoadTO) {// 同步最后加载时间
                    autoLoadTO.setLastRequestTime(System.currentTimeMillis())//
                            ...
                            .setExpireAt(newCacheWrapper.getExpireAt())
    ...
    public void deleteCache(DeleteCacheAopProxyChain jp, CacheDelete cacheDelete, Object retVal) throws Throwable {
        ...
        try {
            for (int i = 0; i < keys.length; i++) {
                ...
                boolean tempHfield = keyConfig.hfield();
                if (!scriptParser.isCanDelete(keyConfig, arguments, retVal)) {
                    continue;
                }
                for (String tempKey : tempKeys) {
                    CacheKeyTO key = getCacheKey(target, methodName, arguments, tempKey, tempHfield,
                            cacheDelete.jedisIndex(), retVal, true);
    ...
    public void writeCache(CacheAopProxyChain pjp, Object[] arguments, Cache cache, CacheKeyTO cacheKey,
        ...
        if (cacheWrapper.getExpire() >= 0) {
            this.setCache(cacheKey, cacheWrapper, method, pjp.getTarget(), arguments);
        }
        ...
        for (ExCache exCache : exCaches) {
            try {
                ...
                int exCacheExpire = scriptParser.getRealExpire(exCache.expire(),
                        exCache.expireExpression(), target, arguments,
                        exResult);
                ...
                if (exCacheExpire >= 0) {
                    this.setCache(exCacheKey, exCacheWrapper, method, target, arguments);
                    if (null != tmpAutoLoadTO) {
                        tmpAutoLoadTO.setExpireAt(now + exCacheExpire).setExpire(exCacheExpire)//
    ...
    public void expireCache(CacheAopProxyChain pjp, Object[] arguments, Cache cache, CacheKeyTO cacheKey,
            CacheWrapper<Object> cacheWrapper) throws Exception {
        if (null == cacheKey) {
            return;
        }
        Method method = pjp.getMethod();
        if (cacheWrapper.getExpire() >= 0) {
            this.expireCacheOnly(cacheKey, cacheWrapper, method, pjp.getTarget(), arguments);
        }
        ExCache[] exCaches = cache.exCache();
        if (null == exCaches || exCaches.length == 0) {
            return;
        ...
        for (ExCache exCache : exCaches) {
            try {
                ...
                AutoLoadTO tmpAutoLoadTO = this.autoLoadHandler.getAutoLoadTO(exCacheKey);
                if (exCacheExpire >= 0) {
                    this.expireCacheOnly(exCacheKey, exCacheWrapper, method, target, arguments);
                    if (null != tmpAutoLoadTO) {
                        tmpAutoLoadTO.setExpireAt(now + exCacheExpire).setExpire(exCacheExpire)//
                                .setLastLoadTime(exCacheWrapper.getLastLoadTime());//
                    }
     ...
     * @return CacheKeyTO
     */
    private CacheKeyTO getCacheKey(Object target, String methodName, Object[] arguments,
            String keyExpression,
            boolean hfieldExpression, int jedisIndex, Object result, boolean hasRetVal) {
        String key = null;
        boolean hfield = hfieldExpression;
        if (null != keyExpression && keyExpression.trim().length() > 0) {
            try {
                ...
                // hfield = scriptParser.getDefinedCacheKey(hfieldExpression, target, arguments,
                // result, hasRetVal);
                // }
            } catch (Exception ex) {
                log.error(ex.getMessage(), ex);
        ...
        return new CacheKeyTO(config.getNamespace(), key, hfield, jedisIndex);
    }
    ...
    private CacheKeyTO getCacheKey(CacheAopProxyChain pjp, Cache cache) {
        ...
        boolean hfieldExpression = cache.hfield();
        int jedisIndex = cache.jedisIndex();
        return getCacheKey(target, methodName, arguments, keyExpression, hfieldExpression,
                jedisIndex, null, false);
    }
    ...
    private CacheKeyTO getCacheKey(CacheAopProxyChain pjp, Object[] arguments, ExCache exCache, Object result) {
        ...
        boolean hfieldExpression = exCache.hfield();
        int jedisIndex = exCache.jedisIndex();
        return getCacheKey(target, methodName, arguments, keyExpression, hfieldExpression,
                jedisIndex, result, true);
    }
    ...
    public void setCache(CacheKeyTO cacheKey, CacheWrapper<Object> result, Method method,
        ...
        cacheManager.setCache(cacheKey, result, method, target, args);
        if (null != changeListener) {
            changeListener.update(cacheKey, result);
        }
    }
    
    public void expireCacheOnly(CacheKeyTO cacheKey, CacheWrapper<Object> result, Method method,
            Object target, Object[] args)
            throws CacheCenterConnectionException {
        cacheManager.expireCache(cacheKey, result, method, target, args);
        if (null != changeListener) {
            changeListener.update(cacheKey, result);
        }
    }
    ...
    public CacheWrapper<Object> get(CacheKeyTO key, Method method, Object target, Object[] args)
            throws CacheCenterConnectionException {
        return cacheManager.get(key, method, target, args);
    }
